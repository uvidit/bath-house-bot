...
jason_geo = ['ща...5 минут турецкий (с)', 'работу работает! .... не то что вы - тунеядцы!', 'позовите Жасона!'
             , 'в пути...', 'скоро будет...', 'подождем его!']
@dp.message_handler(commands=['jason'])
async def send_welcome(message: types.Message):
    await message.reply(f"Где Жасон? (с)\n{jason_geo[randint(0, len(jason_geo) - 1)]}")
...
async def crypt(message: types.Message):
    await message.reply(f"Почем крипта для народа?\n\n{get_latest_coin_data('btc-bitcoin')}\n{get_latest_coin_data('eth-ethereum')}")

@dp.message_handler(regexp='(crypt|Crypt|CRYPT|крипт|Крипт|КРИПТ|биток|Биток|БИТОК|иткоин|ИТКОИН|Эфир|эфир|ЭФИР|itcoin|ITCOIN|thereum|THEREUM)')
...
async def jason_mantion(message: types.Message):
    await message.reply(text="а где Жасон!? (с)")
...
@dp.message_handler(regexp='(((J|j)(oke|OKE))|((Ш|ш)(ут(ей|)к|УТ(ЕЙ|)К))|(чак|Чак|ЧАК|Chuck|CHUCK|chuck)|(NOR|Nor|nor))')
